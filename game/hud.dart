...
class Hud extends PositionComponent with ParentIsA<Viewport>, HasGameReference {
  Hud({
    ...
    this.onPausePressed,
          ...
          scale: Vector2.all(SkiRunnerGame.isMobile ? 0.6 : 1.0),
        ),
  ...
  final _life = TextComponent(
    ...
    textRenderer: TextPaint(
      style: const TextStyle(
        color: Colors.black,
        fontSize: SkiRunnerGame.isMobile ? 8 : 10,
      ),
    ),
  );
  ...
  Future<void> onLoad() async {
    _player.position.setValues(
      16,
      SkiRunnerGame.isMobile ? 10 : parent.virtualSize.y - 20,
    );
  ...
  void update(double dt) {
    if (input?.active ?? false) {
      // FIX 2: Removed 'var d =' assignment
